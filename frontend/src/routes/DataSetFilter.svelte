<script lang="ts">
    ...
    interface DataRow { left: DataItem | null; right: DataItem | null; }

    const data: DataRow[] = [
        { left: { id: "inspections", name: "Inspections" }, right: { id: "inspection_items", name: "Inspection Items" } },
        { left: { id: "templates", name: "Templates" }, right: { id: "template_permissions", name: "Template Permissions" } },
        ...
        { left: { id: "schedule_assignees", name: "Schedule Assignees" }, right: { id: "schedule_occurrences", name: "Schedule Occurrences" } },
        { left: { id: "actions", name: "Actions" }, right: { id: "issues", name: "Issues" } },
        { left: { id: "action_timeline_items", name: "Action Timeline Items" }, right: { id: "issue_timeline_items", name: "Issue Timeline Items" } },
        { left: { id: "action_assignees", name: "Action Assignees" }, right: { id: "issue_assignees", name: "Issue Assignees" } },
        { left: { id: "assets", name: "Assets" }, right: { id: "training_course_progresses", name: "Training course completions" } },
    ]

    const allItems: DataItem[] = data.flatMap(row => [row.left, row.right].filter((x): x is DataItem => x !== null));
                ...
                <div class="flex h-11 border-t border-border">
                    {#if left}
                        <div class="flex w-1/2 items-center px-3">
                            <input type="checkbox" class="size-4 accent-primary" onclick={toggleHeaderCheckbox} bind:group={($shadowConfig as any)["Export"]["Tables"]} value={left.id}/>
                    ...
                    {#if right}
                        <div class="flex w-1/2 items-center px-3">
                            <input type="checkbox" class="size-4 accent-primary" onclick={toggleHeaderCheckbox} bind:group={($shadowConfig as any)["Export"]["Tables"]} value={right.id}/>
                            <img class="ml-4 size-4" src="../images/template-icon.svg" alt="dataset"/>
