...
<div class="flex h-full flex-col items-center">
	...
	<p class="mt-4 mb-3 text-sm text-muted-foreground">
		...
		<button class="cursor-pointer text-primary hover:underline" onclick={openURL}>SafetyCulture account</button>.
	</p>
	...
	<div class="flex items-start gap-2">
		<div class="relative w-[340px]">
			<Input
				...
				class="pr-9 {displayValidationError ? 'border-destructive focus-visible:ring-destructive/50' : ''}"
			/>
			<button
				type="button"
	...
	{#if displayBadApiKeyErr}
		<div class="mt-2 max-w-md text-xs">
			...
			<p class="text-muted-foreground">It looks like your token may be expired after 30 days of inactivity. Please generate a new token and try again.</p>
		</div>
	{/if}
	...
	{#if displayConnectionErr}
		<div class="mt-2 max-w-md text-xs">
			...
			<p class="text-muted-foreground">It looks like you are not connected to the internet or behind a firewall. Please check your connection and try again.</p>
		</div>
	{/if}

	<section class="mt-4 max-w-lg">
		<div class="flex gap-3 rounded-lg border border-border bg-muted/50 p-3 text-xs text-muted-foreground">
			...
			<div>
				...
				<p class="leading-5">All files (apart from SQL) you export will be stored in the same place on your computer or server as the SafetyCulture Exporter. If you want to change where your files get exported, please move the SafetyCulture Exporter file itself to that place.</p>
			</div>
		</div>
	</section>
</div>
