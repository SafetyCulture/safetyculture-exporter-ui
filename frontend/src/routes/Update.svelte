<script lang="ts">
  ...
  let updateMessage = $state(
    'Please wait until we update your application from ' +
  ...
  TriggerUpdate($latestVersion['download_url'])
    .then((result: boolean) => {
      if (result === true) {
...
<div class="flex h-screen flex-col items-center">
  ...
  <div class="flex grow flex-col items-center justify-center">
    ...
    {#if updateStatus === 'failed'}
      {#if $latestVersion['os'] === 'darwin'}
        <p class="pt-2 text-sm text-muted-foreground">
          SafetyCulture Exporter must be moved into the Applications folder in order for the
          auto-update to work
        </p>
      {/if}
      <button
        class="cursor-pointer pt-2 text-sm text-primary hover:underline"
    ...
    <div class="flex gap-2 pt-8">
      <Button variant="default" onclick={() => push('/welcome')} disabled={!cancelActive}
        >Cancel</Button
      >
